...
4. **喜欢页沉浸式大图（v33 新增）**：一屏一个宝贝（左右滑动 + ◀▶ 按钮 + 圆点切换 + 第 X/N 件计数），深色展柜背板 + 射光灯效；含"查看详情"按钮
5. **喜爱展柜分享（v33 新增）**：两种分享方式——
   - **🏛 海报**：`Poster.favPoster()` 生成深色博物馆展柜风格静态海报（深色背景 + 鎏金标题 + 射灯光晕 + 铜牌铭牌），存 JPG 到相册/微信
   - **🔗 动态展厅**：生成公开可访问的动态网页链接 `gallery.html?data=<编码数据>`，任何人（无需登录）打开即见深色展柜 + 射灯扫光 + 旋转光晕动画，左右滑动切换；数据只含公开信息（名称/图片URL/分类/品种/尺寸），图片 URL 公开可访问（Supabase 存储桶 public read）
