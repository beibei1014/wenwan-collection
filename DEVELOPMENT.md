...
59. **历史首次盘玩时间自动回填（v79）**：对**今天之前就已经在盘**的老串（有 `playCount>0` 或 `lastPlayedAt`，但 `firstPlayedAt` 为空），登录后自动回填 `firstPlayedAt` 为**系统里已有的最早盘玩记录**——优先取打卡历史 `playDays` 最早的日期，没有打卡历史则取所有宝贝 `lastPlayedAt` 里最早的那个；系统里完全没有盘玩记录时不回填；已有 `firstPlayedAt` 的串不会被覆盖（`backfillFirstPlayed()`，与 `backfillColors()` 同为登录后静默后台任务）。缓存 v79
